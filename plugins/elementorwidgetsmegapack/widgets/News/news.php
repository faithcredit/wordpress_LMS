 ...
 * @since 1.0.0
 */
class News_Layouts extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'News Layouts', 'elementorwidgetsmegapack' );
	}
	...
	protected function render() {
		...
		if($columns == '1') : $columns_class = 'col-xs-12'; $container_class = 'newslayouts-np-vc-news-1-col'; endif;
		if($columns == '2') : $columns_class = 'col-xs-6'; $container_class = 'newslayouts-np-vc-news-2-col'; endif;
		if($columns == '3') : $columns_class = 'col-xs-4'; $container_class = 'newslayouts-np-vc-news-3-col'; endif;		
		...
		if($pagination == 'load-more') : 
			$class_load_more = 'newslayouts-np-load-more-news-'.$news_style.''; 
			$class_item_load_more = 'newslayouts-np-item-load-more-'.$news_style.'';
		endif;
		// #Load More
		...
		echo '<div class="newslayouts newslayouts-np-vc-element-news '.esc_html($class_load_more).' newslayouts-np-news-'.$news_style.' '.esc_html($container_class).' newslayouts-np-vc-element-news-'.esc_html($instance).' element-no-padding">';
		
		echo '<div class="newslayouts-np-vc-element-news-article-container">';
		...
		if($pagination == 'load-more') : 
			...
			wp_enqueue_script(
				'newslayouts-np-load-news',
				BLE_URL . 'assets/js/newslayouts.js',
				array('jquery'),
				'1.0',
				true
			);		
			...
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				if($news_style == 'style1') {
					...
					if($count == '0') :			
						...
						echo '<article class="item-posts first-element-posts col-xs-8 '.esc_html($class_item_load_more).ewmp_animate_class($addon_animate,$effect,$delay).'>';
				...
				if($news_style == 'style6') {
					if($count < $columns) :
							...
							echo '<div class="article-info">';
								...
								echo '<div class="article-info-bottom">';		
										...
										if($columns != 1) :
											...
											echo '<p class="article-excerpt" '.$css_text.'>' . ewmp_get_news_excerpt(350,'on',$css_link) . '</p>';						
										endif;
										echo '<div class="clearfix"></div>';	
								echo '</div>';
							echo '</div>';
						...
						echo '<article class="item-news first-element-news others-post '.esc_html($columns_class).' '.esc_html($class_item_load_more).' '.esc_html($class_odd).ewmp_animate_class($addon_animate,$effect,$delay).'>';
							echo '<div class="article-image col-xs-5">';
								if($columns != 1) :
									echo ewmp_get_thumb('ewmp-vc-header-small');
								else :
		...
		/**********************************************************************/ 
		if($pagination == 'yes') {
				echo '<div class="clearfix"></div><div class="newslayouts-np-news-display-'.esc_html($instance).' newslayouts-np-vc-pagination">';
				if($pagination_type == 'numeric') {
					echo ewmp_posts_numeric_pagination($pages = '', $range = 2,$loop,$paged,$css_current_pag_num,$css_link);
				} else {
					echo '<div class="newslayouts-np-pagination-normal">';
